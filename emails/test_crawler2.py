...
class MailSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        unique_emails = set(emails)  # Use a set to remove duplicates

        # Yield only the relevant emails
        for email in unique_emails:  # Iterate over unique emails
            if re.match(email_pattern, email):  # This check is now somewhat redundant due to the regex match above
                yield {
                    'url': response.url,
                    'email': email
                }